...
public class LaunchCalculator {
    public int calculateTotalPrice(Spaceportt spaceport, int passengerCount) {
...
abstract class Spaceportt {
    public abstract int calculateTotalPrice(int passengerCount);
}
class JupiterStar extends Spaceportt {
public int calculateTotalPrice(int passengerCount){
    int jupiterStarPrice = (passengerCount * 2) + 500;
    return jupiterStarPrice;
}
}
class MercuryBeat extends Spaceportt {
    public int calculateTotalPrice(int passengerCount){
        int mercuryBeatPrice = (passengerCount * 15) + 0;
        return mercuryBeatPrice;
    }
}
class SunHeaven extends Spaceportt {
    public int calculateTotalPrice(int passengerCount){
        int sunHeavenPrice = (passengerCount * 0) + 2000;
        return sunHeavenPrice;
    }
}